...
<template>
  <v-container>
    <v-form @submit.prevent="onSubmit">
      <v-text-field v-model="question_text" v-bind="nameAttrs" label="Question"></v-text-field>
      <span>{{ errors.question_text }}</span>

      <div>
        <div v-for="(option, index) in fields" :key="index" class="mb-2">
          <v-text-field :label="`Option ${index + 1}`" v-model="option.text"></v-text-field>
